...
import { createImage, parseImageAttributeString } from "./util"
 ...
 */
const remarkD2: Plugin = function (opts?: typeof DEFAULT_OPTIONS): Transformer {
  ...
  return (tree, file) => {
    ...
    if (file.path !== undefined || file.path !== null) {
      ...
      relDir = path.isAbsolute(file.path) ? path.relative(file.cwd, relDir) : relDir
    }
    ...
    visit(tree, "code", (node) => {
      ...
      const d2 = spawn("d2", [...opts.defaultD2Opts, "-", `${path.join(compileDir, image)}`])
